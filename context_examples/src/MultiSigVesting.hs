...
mkValidator dtm () ctx = ((beneficiary1 dtm `elem` sig) && (to (deadline dtm) `contains` range)) ||
				   	   	 ((beneficiary2 dtm `elem` sig) && (from (deadline dtm) `contains` range))
			where
				info :: TxInfo
...
typedValidator :: Scripts.ScriptInstance Vesting
typedValidator = Scripts.validator @Vesting
