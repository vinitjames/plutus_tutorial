...
module MixedInputVesting where
...
mkValidator phk st () ctx =
            traceIfFalse "Beneficiary not Valid" checkPubKey &&
            ...
            where
                ...
                checkPubKey :: Bool
                checkPubKey = phk `elem` (txInfoSignatories info)

                checkDeadline :: Bool
                checkDeadline = contains (from st) (txInfoValidRange info)
...
typedValidator :: PubKeyHash -> Scripts.ScriptInstance Vesting
typedValidator phk = Scripts.validator @Vesting
